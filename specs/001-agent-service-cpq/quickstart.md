...
5. Complete the required intake in the ordinary UI. Confirm every value is labeled `Human`, and an incompatible option or impossible date produces an actionable conflict.
...
9. Review the server-authored final summary. Change the slot or contact after reviewing it and confirm the prior confirmation is invalid. Review again and explicitly confirm the current summary.
...
3. have the agent create and populate a scope, then verify the visible canvas shows `Agent` provenance;
...
|---|---|
| `pnpm contract:check` | OpenAPI parses; generated Zod/JSON schemas agree; exact six tools and correct state effects; RFC 9457 errors validate; unknown fields are rejected. |
| `pnpm test:unit` | Scope normalization, missing/conflict detection, all four pricing rule types, explicit rounding, deterministic order, quote/confirmation fingerprints, and state transitions pass. |
...
- stale `If-Match`/`expectedRevision`;
